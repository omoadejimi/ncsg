---
title: Plain language
description: Write so people understand the first time, on the first screen, without help.
keywords: [plain english, readability, simple words, kindly, jargon]
---

## Why plain language

Plain language is the single highest-leverage content decision a Nigerian product team can make. English is the language of Nigerian interfaces, but it is a second, third or fourth language for most users. Every complicated sentence quietly excludes someone.

Plain language is also faster for expert users. Nobody has ever complained that a bank explained a charge too clearly.

## Guidelines

### Use short, common words

Prefer the word a market trader and a lawyer both use.

| Instead of | Write |
| --- | --- |
| utilise | use |
| commence | start |
| terminate | end, cancel |
| insufficient | not enough |
| beneficiary | the person you're sending to |
| remuneration | pay |

### Keep sentences short

Aim for an average of 15 words and one idea per sentence. If a sentence needs a comma to breathe, consider making it two.

### Retire "kindly"

"Kindly" is the most overworked word in Nigerian corporate English. In interfaces it adds length without adding politeness — the polite thing is to be clear.

<DoDont>
<Do>
Enter the OTP we sent to 0803 *** 4567.
</Do>
<Dont>
Kindly input the One-Time Password that has been dispatched to your registered mobile number.
</Dont>
</DoDont>

### Avoid officialese

"Do the needful", "same has been forwarded", "as per", "revert" — this register signals bureaucracy, and bureaucracy signals delay. Say the specific thing instead.

### Front-load the point

Put the most important information first: in the sentence, in the paragraph, on the screen. Users on slow connections may only ever see the top of the page.

### Write how you would say it

Read your copy aloud. If you wouldn't say it to a customer standing in front of you, don't ship it.

## Testing plainness

- Read the copy to someone outside the team. Ask them to say it back in their own words.
- Check reading level — aim for roughly age 12–14 comprehension for general audiences.
- Watch session recordings or usability tests for re-reading: hesitation on a sentence is data.

## Related guidance

- [Nigerian English](/docs/nigeria/nigerian-english)
- [Error messages](/docs/patterns/error-messages)
- [Inclusive writing](/docs/foundations/inclusive-writing)
